...
<br>The design using the LUTs is a "bit" **off!** (although still acceptable)<br>
The culprit is the fixed gate voltage bias $V_{GS}$ (that in turn causes the $V_{DS}$ to be off from the desired 0.6V)<br> 
In a practical implementation, the DC gate voltage is set using an auxiliary feedback circuit that computes $V_{GS}$ such that <br>
$V_{DS}$=0.6V<br>

If we set $V_{GS}=0.4780 V$ the simulation (**igs_ex3_4a_fix.sch**) results are:<br><br>
